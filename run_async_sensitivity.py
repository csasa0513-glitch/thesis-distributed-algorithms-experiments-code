"""
Q3 (async sensitivity): effect of rewiring probability p_rew on the
asynchronous gossip algorithm.

Outputs:
    async_sensitivity.csv
    async_sensitivity_trajectories.npz
...
# Hard-coded §6.4 invariants. See run_sync_sensitivity.py for context.
N_WS: int = 50
WS_K_FIXED: int = 6
...
def _run_sweep(family, param_values, graph_builder, x_star, master_rng):
    rows = []
    curves = {}
    events_per_rep, rel_err_per_rep, x0_per_rep = [], [], []
    ...
    for p_rew in param_values:
        ...
        for r in range(N_REPS):
            ...
            out = run_async(G, x_star, max_events=MAX_EVENTS, eps=0.0,
                            x0=x0, rng=gossip_rng)
            if events_axis is None:
                events_axis = out["events"]
            ...
            events_per_rep.append(out["events"])
        ...
        curves[f"{family} {p_rew}"] = (events_axis, mean, low, high)
    ...
    traj = {
        "events_per_rep": np.stack(events_per_rep),
...
def main():
    ...
    print(f"[start] sync run must log the SAME sha256 prefix.", flush=True)
    ...
    print("=" * 70, flush=True)
    print("run_async_sensitivity.py", flush=True)
    print(f"  N={N_WS}, K={WS_K_FIXED}, R={N_REPS}, MAX_EVENTS={MAX_EVENTS}",
          flush=True)
    ...
    print(f"\nSaved async_sensitivity.csv ({len(df)} rows)")
    ...
    np.savez(C.RESULTS / "async_sensitivity_trajectories.npz", **traj)
    print(f"Saved async_sensitivity_trajectories.npz")
    ...
    fig.suptitle("Async sensitivity to rewiring probability $p_{rew}$, "
                 "$WS(50,\\,6,\\,p_{rew})$")
    fig.tight_layout()
